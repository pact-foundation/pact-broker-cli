...
use crate::cli::{
    ...
    pactflow::main::{
        provider_contracts,
        subcommands::{
            add_publish_provider_contract_subcommand, add_publish_provider_contracts_subcommand,
        },
    },
};
pub fn add_pactflow_client_command() -> Command {
    Command::new("pactflow")
        ...
        .subcommand(add_publish_provider_contracts_subcommand())
}

pub fn run(args: &ArgMatches) -> Result<serde_json::Value, i32> {
    match args.subcommand() {
        ...
        Some(("publish-provider-contracts", args)) => {
            let res = provider_contracts::publish_multiple::publish_multiple(args);
            if let Err(err) = res {
                handle_error(err);
                Err(1)
            } else {
